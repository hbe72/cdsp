...
TEST(fft, forward_q4_20)
{
    ...
    using complex = cdsp::complex<q4_20>;
    using complex_vector = cdsp::complex_vector<q4_20>;
    complex_vector vec(fftSize);
    ...
    for (unsigned int impulseLoc = 0; impulseLoc < fftSize; ++impulseLoc)
    {
        vec.fill(complex(0.0, 0.0));
        vec.set_at(impulseLoc, complex(1.0, 0.0));
        int exponent = cdsp::fft::fft<q4_20>(vec);
        ...
        for (unsigned int i = 0; i < fftSize; ++i)
        {
            ...
            unsigned int index = (i * (impulseLoc)) % fftSize;
            double ref_angle = twoPi * (double)index / (double)fftSize;
            auto ref = cdsp::complex<double>(std::cos(ref_angle), -std::sin(ref_angle));
...
TEST(fft, reverse_q4_20)
{
    ...
    for (unsigned int impulseLoc = 0; impulseLoc < fftSize; ++impulseLoc)
    {
        vec.fill(complex(0.0, 0.0));
        vec.set_at(impulseLoc, complex(1.0, 0.0));
        int exponent = cdsp::fft::ifft<q4_20>(vec);
        auto scale = pow(2.0, exponent);
        for (unsigned int i = 0; i < fftSize; ++i)
        {
            // For given impulse location FFT result can be calculated easily
...
TEST(fft, real_forward_double)
{
    ...
    for (unsigned int impulseLoc = 0; impulseLoc < fftSize; ++impulseLoc)
    {
        ...
        in[impulseLoc] = 1.0;
        cdsp::fft::real_fft<double>(in, out);
...
TEST(fft, real_forward_q4_20)
{
    ...
    for (unsigned int impulseLoc = 0; impulseLoc < fftSize; ++impulseLoc)
    {
        ...
        int exponent = cdsp::fft::real_fft<q4_20>(in, out);
        auto scale = pow(2.0, exponent);
        for (unsigned int i = 0; i < 1 + fftSize / 2; ++i)
        {
            // For given impulse location FFT result can be calculated easily
...
TEST(fft, forward_random_q4_20)
{
    ...
    cdsp::complex_vector<q4_20> vec_fix(fftSize);
    cdsp::complex_vector<q4_20> vec_fix_real(1 + fftSize / 2);
    ...
    int exponent_fix = cdsp::fft::fft<q4_20>(vec_fix);
    int exponent_fix_real = cdsp::fft::real_fft<q4_20>(input_fix, vec_fix_real);
